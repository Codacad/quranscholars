...
const sortCourses = (items, sort) => {
  ...
  if (sort === "price-low")
    return copy.sort((a, b) => a.price.current - b.price.current);
  return copy.sort((a, b) => b.totalStudents - a.totalStudents);
};

const getRecordedCourses = ({
  search = "",
  category = "All",
  level = "All",
  sort = "popular",
...
const getRecordedCourseBySlug = (slug) =>
  courses.find((course) => course.slug === slug) || null;

const getFeaturedRecordedCourses = (limit = 4) =>
  courses.filter((course) => course.featured).slice(0, limit);
...
const recordedCourseCategories = [
  "All",
  ...new Set(courses.map((course) => course.category)),
];

const recordedCourseLevels = [
  "All",
  "Beginner",
  "Intermediate",
  "Advanced",
